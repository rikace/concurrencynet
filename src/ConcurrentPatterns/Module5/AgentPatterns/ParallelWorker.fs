...
module ParallelWorker =
    ...
    let parallelCoordinator n f =
        MailboxProcessor.Start(fun inbox ->
            ...
            // missing code
            // create a recursive/async loop with an
            // internal state to track the child-agent index
            // where a message was sent last

            let loop i =  async.Return(()) // replace with missing code
    ...
    let parallelWorker n (f:_ -> Async<unit>) =
        // TODO LAB
        // use the "parallelCoordinator" for the implementation
