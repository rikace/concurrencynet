...
let printerAgent =
    Agent<Msg<string, string>>.Start((fun inbox -> async {
      while true do
        ...
        match msg with
        | Item(t) -> printfn "%s" t
        | Mailbox(agent) -> failwith "no implemented"}))
...
module ParallelWebCrawler =
    ...
    type Message =
        | B of bool
        | I of int
        | M of AsyncReplyChannel<int>

//    type Box<'a> = MailboxProcessor<'a>
//
//    let mailBox = Box.Start(fun inbox -> async {
//        let! msg = inbox.Receive()
//        printfn "This is my message %A" msg
//        match msg with
//        | B v when v = true -> printfn "this message is %A" v
//        | I v -> printfn "this message is %A" v
//        | M ch -> ch.Reply(15)
//
//        })
//    mailBox.Post (B false)
//    mailBox.PostAndReply(fun ch -> M ch)
